...
export class VacancyStateRepository extends BaseRepository implements IVacancyRepository {
    ...
    async getVacancyState({ id }: { id: IVacancyID }): Promise<Either<DataExceptions, VacancyState[]>> {
        try {
            const response = await this.client.get<IVacancyServiceDTO | IVacancyServiceError>({
                url: `${NetworkConstants.BASE_API_PORT}${NetworkConstants.BASE_API_NAMESPACE}/v1/candidate-status/${id}`,
